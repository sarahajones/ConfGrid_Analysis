...
high <- subset(confidence, zConf > -5) # 10 trials have a z score of less than -5
low <- subset(confidence, zConf > 5) #4 trials here have z score of more that 5
rm(high, low)

#remove those 14 trials just to improve the color gradient in the viz
confSubset <- subset(confidence, zConf < 5 & zConf > -5) #still have 9169 trials here
...
x4 <- x1
x4 <- ((x4/(2/3))*100) #set it into 0-100 space ###2/3?
...
DistCol <- abs((1/3) - y[401:601, 1:200]) #pick color here
DistSize <- abs((1/3) - x[401:601, 1:200]) #pick size here
x4[401:601, 1:200] <- (((pythagorean(DistCol,DistSize))/(2/3))*100) ##2/3??
...
#meaningless - too noisy, too many data points

#look within each grid box on average to get a better feel
modelData <- subset(modelData, is.na(minimumDistance) ==FALSE) #lose 9 trials where confidence somehow flooped (check why later)

modelGroupped <- modelData %>%
  group_by(grid_box) %>%
  dplyr:: summarise(mean = mean(confidence))
modelMin <- modelData %>%
  group_by(grid_box) %>%
  dplyr:: summarise(Minmean = mean(minimumDistance))
modelMax <- modelData %>%
  group_by(grid_box) %>%
  dplyr:: summarise(Maxmean = mean(maximumDistance))
modelSum <- modelData %>%
  group_by(grid_box) %>%
  dplyr:: summarise(Summean = mean(summedDistance))

modelGroupped$Minmean <- modelMin$Minmean
modelGroupped$Maxmean <- modelMax$Maxmean
modelGroupped$Summean <- modelSum$Summean
rm(modelSum, modelMax, modelMin)

cor.test(modelGroupped$mean, modelGroupped$Minmean)
cor.test(modelGroupped$mean, modelGroupped$Maxmean)
cor.test(modelGroupped$mean, modelGroupped$Summean)


library(Hmisc)
data <- modelData[c("confidence", "minimumDistance","maximumDistance", "summedDistance")]
